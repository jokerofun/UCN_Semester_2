...
public class UpdatePositionDialog extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public UpdatePositionDialog(Position position, SynchronizationController synchronizationController) {
		try {
		...
		setTitle("Position Edit Menu - Position name: " + position.getPositionName());
		...
		contentPanel.setLayout(new BorderLayout(0, 0));
		{
			...
			{
				positionNameTextField = new JTextField("" + position.getPositionName());
			...
			{
				clearanceTextField = new JTextField("" + position.getClearance());
	...
	private void confirm() {
		try {
			if (regexChecker()) {
				positionController.updatePosition(positionNameTextField.getText(), clearanceTextField.getText());
